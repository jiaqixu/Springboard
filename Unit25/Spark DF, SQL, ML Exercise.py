...
gbParamGrid = (ParamGridBuilder()
             .addGrid(gb.featureSubsetStrategy, ['auto', 'sqrt', 'log2'])
...
# look at best params from the CV
print(gbCvModel.bestModel._java_obj.getFeatureSubsetStrategy())
...
attrs = sorted((attr["idx"], attr["name"]) for attr in (chain(*lrPredictions.schema[lrModel.summary.featuresCol].metadata["ml_attr"]["attrs"].values())))

lrCvFeature = pd.DataFrame([(name, lrCvModel.bestModel.coefficients[idx]) for idx, name in attrs],columns=['feature_name','feature_coefficient'])
print(lrCvFeature.sort_values(by=['feature_coefficient'],ascending =False))
